...
<br>이러면 라이센스 문제등이 생길 수도있고 파일이 너무 커진다.
  ...
  - 다운 받은 모델이나 소스 , 에셋들은 (Imports/Resources) 임의의 폴더안에 넣고 Ignore에 추가해주자.
...
**4. 깃허브에서 소스파일을 유니티로 연결하기**
	...
	1. 파일을 다운 받거나 비주얼스튜디오로 열기를 선택한다.
	
	2. 그후 유니티 허브를 키고 Open 을 눌러서 이미있는 프로젝트 열기를 해준다.

	3. 그렇게 실행시키면 기본 유니티 라이브러리 등 이 자동으로 임포트 된다.

	4. 추가로 에셋들이 필요한 것은 따로 받아서 넣어준다.
